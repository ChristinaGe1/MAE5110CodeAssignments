...
**Due date:** Wednesday, September 16th, 11:59PM (midnight)
...
We're still going to model stance as a simple inverted pendulum, but instead of defining the angle $\alpha$ between the stance and swing leg by the number of spokes in a rimless wheel, we will treat that angle as a control input that can be freely chosen each step.
In other words, at the start of each stance phase, you get to instantly choose the next landing angle-of-attack $\alpha$ and instantly teleport your massless leg to the correct angle.

<!-- ## Control as a look-up table

We'll need convert the dynamics into a graph-search to solve this with DP. For solving for the landing angle-of-attack $\alpha$, this lends itself quite naturally since it is already a discrete-time decision (once per step). For the continuous-time torque $\tau$, we'll need to discretize the system. -->
...
Create a new model, `InvertedPendulumWalker`, based on the rimless wheel, and add parameter indicating the ankle torque `ankle_torque`, which should sum to the pendulum dynamics (set this as 0 by default). The rest of the model details shouldn't need to change.
...
In your script, you'll also define the following bounds: $\alpha \in \left[ 0, \frac{\pi}{4} \right]$, and $\tau \in \left[-0.1mg\ell, 0.05mg \ell \right ]$, which your controller will need to respect.
...
Any time your systems' state enters this region of attraction, you're done!
...
You should have reaqlized from your sketches, now that we are allowing $\alpha$ to change, the touch-down guard changes with the control input.
Let's make our lives easier again by picking a new, simpler Poincaré section.
Recall, the Poincaré section needs to be transverse to the system flow: a good choice here will be to take the slice $\theta = 0$.
...
Next, you want to create a look-up table so that, for any given initial condition, you can find the sequence of footsteps that brings you into the RoA of your standing controller.
...
For the states, sweep intial angular velocities between zero and $\dot{\theta} = \sqrt{2 g \ell}$, which is based on reaching a [Froude number of 2](https://www.sciencedirect.com/science/article/pii/S0966636204000268?casa_token=meVbupO-zQ8AAAAA:XxAS1QOrU4YuBgSAPBe1Fm3FsncZetZ8anQn7YifLLJiyHv-jTBFn3jlWgm2LC0xoMnizUqCFA).
...
Open a pull request (PR) from that branch into the `main` branch of your own fork, and submit the link to that PR on Canvas.
...
**Note:** you should absolutely avoid committing binary files (non-text files) such as images in git.
